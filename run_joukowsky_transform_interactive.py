...
class JoukowskyTransform:
    def __init__(self):
        ...
        self.cy = 0.0
        self.rad = 1.0
        self.circle = Circle(self.cx,self.cy,self.rad,self.n)
    
    def get_circle(self):
        return self.circle
    
    def move_circle(self,tx,ty):
        self.cx += tx
        self.cy += ty
        self.circle[:,0] += tx
        self.circle[:,1] += ty

    def set_radius_circle(self,rad):
...
circ = Circle(0,0,rad,n)
ax_circ = ax.plot(circ[:,0],circ[:,1])
# plt.show()
